...
@pytest.fixture
def model_id():
    # 原 A5 staff 路径是 /data/a5-alignment/models/Qwen2.5-Math-1.5B（Together
    # cluster 内部路径）。本地把模型放在 /data/CS336-use/models/...，可以用
    # TINY_TRAINING_RL_MODEL 环境变量覆盖。
    return os.environ.get(
        "TINY_TRAINING_RL_MODEL",
        "/data/CS336-use/models/Qwen2.5-Math-1.5B",
    )
...
@pytest.fixture
def model(model_id):
    # snapshot 在 fp32 录制，新版 transformers 默认按 config 加载 bf16，需强制 fp32
