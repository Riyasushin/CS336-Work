...
class ToyFSDPModel(nn.Module):
    """Simple model using tiny_training_basic RLinear / REmbedding / RRMSNorm for FSDP testing."""

    def __init__(self, vocab_size=100, d_model=64, d_ff=128):
        super().__init__()
        from tiny_training_basic.layers import REmbedding, RLinear, RRMSNorm

        self.embedding = REmbedding(vocab_size, d_model)
        self.norm1 = RRMSNorm(d_model)
        self.linear1 = RLinear(d_model, d_ff)
        self.norm2 = RRMSNorm(d_ff)
        self.linear2 = RLinear(d_ff, d_model)
        self.lm_head = RLinear(d_model, vocab_size)
...
def _apply_mixed_precision_hooks(model, compute_dtype):
    ...
    for mod in model.modules():
        if not isinstance(mod, (RLinear, REmbedding)):
            continue
        ...
        mod.weight.register_post_accumulate_grad_hook(make_grad_hook(mod, isinstance(mod, RLinear)))
