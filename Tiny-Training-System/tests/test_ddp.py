...
from tiny_training_system.ddp import DDPBucketed
...
def _naive_ddp_factory(module):
    return get_ddp(module)


def _bucketed_ddp_factory(module):
    # 默认 25 MB 桶；ToyModel 总参数量远小于 25 MB，实际上会落成 1 个桶。
    return DDPBucketed(module, bucket_size_mb=25.0)
...
def test_DistributedDataParallel(model_class):
    ...
    mp.spawn(
        ...
        args=(world_size, model_class, _naive_ddp_factory),
        nprocs=world_size,
        join=True,
    )
...
@pytest.mark.parametrize("model_class", [ToyModel, ToyModelWithTiedWeights])
def test_DistributedDataParallel_Bucketed(model_class):
    world_size = 2
    mp.spawn(
        _test_DistributedDataParallel,
        args=(world_size, model_class, _bucketed_ddp_factory),
        nprocs=world_size,
        join=True,
    )


def _test_DistributedDataParallel(rank: int, world_size: int, model_class: type[torch.nn.Module], ddp_factory):
    ...
    ddp_model = ddp_factory(ddp_base)
